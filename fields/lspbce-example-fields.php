...
?>
	<div>
		...
		<div class="meta-row">
			...
			<div class="meta-td">
				<textarea rows="7" cols="50" name="example-details" id="example-details"><?php if ( ! empty($lspb_stored_data['example-details']) ) echo esc_attr( $lspb_stored_data['example-details'][0] ); ?></textarea>
			</div>
		</div>
	</div>
<?php	
...
function lspb_example_meta_save( $post_id )
{
	...
	{
		update_post_meta( $post_id, 'example-details', sanitize_text_field( $_POST['example-details'] ) );
	}
}
